...
def plot_omp_comparison2(filename):
    ...
    plt.yscale('log')
    plt.title('[M3]Time vs. Matrix Sizes for Different Threads, BS=256 (avg of 5 runs)')
    plt.legend(title='Thread configuration')

    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    plt.savefig(f'../data/graphs/omp_comparison2_256.png')
    plt.show()
...
def plot_omp_performance_comparison(filename):
    ...
    plt.ylabel('Speedup')
    plt.title('[M3]Speedup Comparison for Different Thread configurations, BS=256 (avg of 5 runs)')
    plt.legend(title='Matrix Sizes', loc='upper left', bbox_to_anchor=(0, 1), prop={'size': 9})
    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    plt.savefig('../data/graphs/omp_speedup_comparison256.png')
    plt.show()
    ...
    plt.yscale('linear')
    plt.title('[M3]Efficiency Comparison for Different Thread configurations, BS=256 (avg of 5 runs)')
    plt.legend(title='Matrix Sizes', loc='upper right', prop={'size': 9})
    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    plt.savefig('../data/graphs/omp_efficiency_comparison256.png')
    plt.show()

def plot_blocksize_comparison1(filename):
    df = pd.read_csv(filename, delimiter=";")
    df_avg = df.groupby(['blocksize', 'mPow_size']).agg({'time(s)': 'mean'}).reset_index()

    # Create a figure and axis for the plot
    plt.figure(figsize=(10, 6))

    # Loop through each matrix size and plot the time vs. block size
    matrix_sizes = df_avg['mPow_size'].unique()
    for matrix_size in matrix_sizes:
        subset = df_avg[df_avg['mPow_size'] == matrix_size]
        plt.plot(subset['blocksize'], subset['time(s)'], marker='o', linestyle='-',label=f"Matrix Size {matrix_size}")
        plt.xticks(subset['blocksize'])
    ...
    plt.xscale('log', base=2)
    plt.legend()
    plt.title('Time vs. Block size for Different Matrix Sizes')
    plt.legend(title='Matrix Sizes', loc='upper right', prop={'size': 6})

    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    if filename == "../data/csv/IMPtime.csv":
        plt.savefig(f'../data/graphs/blocksize_implicit1.png')
    elif filename == "../data/csv/SEQtime.csv":
        plt.savefig(f'../data/graphs/blocksize_seq1.png')
    plt.show()

def plot_blocksize_comparison2(filename):
    df = pd.read_csv(filename, delimiter=";")
    df_avg = df.groupby(['blocksize', 'mPow_size']).agg({'time(s)': 'mean'}).reset_index()
    ...
    # Loop through each matrix size and plot the time vs. block size
    blocks = df_avg['blocksize'].unique()
    for block in blocks:
        subset = df_avg[df_avg['blocksize'] == block]
        plt.plot(subset['mPow_size'], subset['time(s)'], marker='o', linestyle='-', label=f"blocksize {block}")

    plt.xlabel('Matrix size pow (2^n)')
    plt.ylabel('log Time (s)')
    plt.yscale('log',)
    plt.title('[M3]Time vs. Matrix Sizes for Different Block Sizes (avg of 5 runs)')
    plt.legend(title='Block Sizes')

    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    if filename == "../data/csv/IMPtime.csv":
        plt.savefig(f'../data/graphs/blocksize_implicit2.png')
    elif filename == "../data/csv/SEQtime.csv":
        plt.savefig(f'../data/graphs/blocksize_seq2.png')
...
def plot_omp_performance(n_threads, time, mPow_size):
    ...
    plt.title(f'Efficiency vs Threads for Matrix of Size 2^{mPow_size} Block-based')
    plt.legend()
...
def plot_imp_optimizations():
    ...
    plt.title('[M3]Time vs Matrix Size (2^n) implicit compare execution, BS=256 (avg of 5 runs)')
    plt.savefig(f'../data/graphs/imp_optimizations_compare_256.png')
    plt.show()
...
def main():
    ...
    # plot_blocksize_comparison1(imp_csvfile)
    # plot_blocksize_comparison1(seq_csvfile)
    # plot_blocksize_comparison2(imp_csvfile)
    # plot_blocksize_comparison2(seq_csvfile)
    # plot_imp_optimizations()  #SELECT ONE BLOCKSIZE
